...
module AddressConstraint =
    ...
    let rec findAddressConstraintsInStmt stmt pars def_state_addrs use_state_addrs =
        ...
        | Seq (s1, s2) ->
            let _ = findAddressConstraintsInStmt s1 pars def_state_addrs use_state_addrs
            findAddressConstraintsInStmt s2 pars def_state_addrs use_state_addrs
        | If (BinOp (op, e1, e2, _), s1, s2, _) ->
            ...
            | _ -> ()
        | While (e, s) ->
            findAddressConstraintsInStmt s pars def_state_addrs use_state_addrs
        | _ -> ()
        
    let rec private findAddressConstraints fname stmt pars glb =
        ...
        let _ = findAddressConstraintsInStmt stmt pars def_state_addrs use_state_addrs
        let _ =
    ...
    let getAddressConstraints (mainFuncs:func list) glb =
        let folder acc func =
            DEFPAIRS <- [||]
            USETRIPLES <- [||]
            ...
            let constraints = findAddressConstraints fname stmt pars glb
