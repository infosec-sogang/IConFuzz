...
type mapKeyConstraint = {
    MappingId : string
    DefKeys : key array
    UseKeys : key array
}

module MappingKeyConstraint =
    ...
    let rec private findMappingVariablesInStmt mapKeyConstraints stmt gvars =
        match stmt with
        | Assign (lv,e,_) -> 
            ...
            if defMappings.Length >0 && useMappings.Length >0 then
                let updatedConstraints = UpdateDefKey mapKeyConstraints defMappings
                UpdateUseKey updatedConstraints useMappings
            else if defMappings.Length >0 then
                UpdateDefKey mapKeyConstraints defMappings
            else if useMappings.Length >0 then
                UpdateUseKey mapKeyConstraints useMappings
            else mapKeyConstraints
        | Seq (s1,s2) ->
            let updatedConstraints = findMappingVariablesInStmt mapKeyConstraints s1 gvars
        ...
        | If (e,s1,s2,_) ->
            let useMappings = findMappingVariablesInExp e gvars
            let updatedConstraints = UpdateUseKey mapKeyConstraints useMappings
    ...
    let getMapKeyConstraints (funcSpecs: FuncSpec list) (mainFuncs: func list) gvars =
        let folder acc func =
