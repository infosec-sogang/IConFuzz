...
let private initializeWithDFA (opt: FuzzOption) =
  ...
  let contSpec, seqs, duchains, implicitChains, addressChains = TopLevel.run opt'
  if List.isEmpty seqs // No DU chain at all.
  then (contSpec, makeSingletonSeeds contSpec, duchains, implicitChains, addressChains)
  else (contSpec, List.map (sequenceToSeed contSpec) seqs, duchains, implicitChains, addressChains)
...
let rec private randFuzzLoop opt contSpec duchains implicitChains addressChains concQ randQ =
  ...
  if Executor.isExhausted () then (concQ, randQ)
       ...
       let newSeeds = RandomFuzz.run seed opt contSpec duchains implicitChains addressChains
       let rewindedSeeds = rewindCursors newSeeds
       let concQ = List.fold ConcolicQueue.enqueue concQ rewindedSeeds
       let randQ = List.fold RandFuzzQueue.enqueue randQ newSeeds
       if opt.Verbosity >= 4 then printNewSeeds rewindedSeeds
       randFuzzLoop opt contSpec duchains implicitChains addressChains concQ randQ

let private repeatRandFuzz opt contSpec duchains implicitChains addressChains concQ randQ randFuzzBudget =
  if opt.Verbosity >= 2 then log "Random fuzzing phase starts"
  Executor.allocateResource randFuzzBudget
  Executor.resetPhaseExecutions ()
  let tcNumBefore = TCManage.getTestCaseCount ()
  let concQ, randQ = randFuzzLoop opt contSpec duchains implicitChains addressChains concQ randQ
  let tcNumAfter = TCManage.getTestCaseCount ()
...
let rec private fuzzLoop opt contSpec duchains implicitChains addressChains concQ randQ =
  ...
  let concQ, randQ = repeatRandFuzz opt contSpec duchains implicitChains addressChains concQ randQ randFuzzBudget
  fuzzLoop opt contSpec duchains implicitChains addressChains concQ randQ
...
let run args =
  ...
  let contSpec, initSeeds, duchains, implicitChains, addressChains =  if opt.DynamicDFA then initializeWithDFA opt
