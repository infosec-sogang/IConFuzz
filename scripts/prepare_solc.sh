...
solc_list=("0.4.16 0.4.17 0.4.18 0.4.19 0.4.20 0.4.21 0.4.23 0.4.24 0.4.25 0.4.26
...
function download_solc() {
    ...
    mv ./solc-static-linux ./solc_$1
    chmod +x ./solc_$1
}

# .solc directory should be under IConFuzz directory. Check the cwd before you run the script.
mkdir -p .solc
cd .solc

for version in $solc_list; do
    download_solc "$version"
done
